...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:uibk="http://igwee.uibk.ac.at/custom/ns" exclude-result-prefixes="xs uibk tei" version="2.0">

    <xsl:param name="fromToPattern">^(.*?)\s+an\s+(.*?)$</xsl:param>
    <xsl:param name="editionStmt">
        <edition>
            <date when="2014-03-25" />
Online Edition Test: Herrscher Korrespondenz</edition>
    </xsl:param>
    <xsl:param name="publicationStmt">
        <p>Testveröffentlichung Herrscher Korrespondenz</p>
    </xsl:param>
        ...
        <titleStmt>
            <title>
                <persName role="sender">
                    <xsl:value-of select="$metadata/uibk:metadata/uibk:sender"/>
                </persName>
                <xsl:text> an </xsl:text>
                <persName role="recipient">
                    <xsl:value-of select="$metadata/uibk:metadata/uibk:recipient"/>
                </persName>
                <xsl:text>, </xsl:text>
                <date>
                    <xsl:value-of select="$metadata/uibk:metadata/uibk:date"/>
                </date>
            </title>
            <author>
                <xsl:value-of select="$metadata/uibk:metadata/uibk:sender"/>
    ...
    <xsl:template match="tei:fileDesc/tei:sourceDesc" mode="insertMetadata">
        <sourceDesc>
            <xsl:apply-templates mode="insertMetadata"/>
            <p>Document ID: <idno>
                <xsl:value-of select="$metadata/uibk:metadata/uibk:id"/>
            </idno>
        </p>
    </sourceDesc>
</xsl:template>
...
<xsl:template name="getMetadata">
    ...
    <uibk:metadata>
        ...
        <uibk:date>
            <xsl:value-of select="$fromToDate//uibk:date"/>
        </uibk:date>
        <uibk:id>
                            ...
                            <xsl:analyze-string select="$title" regex="{$fromToPattern}">
                                <xsl:matching-substring>
                                    <uibk:fromToDate>
                                        ...
                                        <uibk:date>
                                            <xsl:value-of select="$table/tei:row[1]/tei:cell[2]"/>
                                        </uibk:date>
                                    </uibk:fromToDate>
                                </xsl:matching-substring>
