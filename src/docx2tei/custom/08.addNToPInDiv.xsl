...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="@* | tei:* | text()" mode="RHaddNToPInDiv">
        <xsl:copy>
            <xsl:apply-templates select="@*" mode="RHaddNToPInDiv"/>
            <xsl:apply-templates mode="RHaddNToPInDiv"/>
        </xsl:copy>
    </xsl:template>

    <xsl:template match="tei:div[@type='commentary' or @type='transcription']/tei:p" mode="RHaddNToPInDiv">
        <xsl:choose>
