<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="/">
    ...
    <xsl:variable name="temp3">
      <xsl:apply-templates mode="RHinsertSeparationAnchor" select="exsl:node-set($temp2)"/>
    </xsl:variable>
    <xsl:variable name="temp4">
      <xsl:apply-templates mode="RHshiftSeparationAnchor" select="exsl:node-set($temp3)"/>
    </xsl:variable>
    <xsl:variable name="temp5">
      <xsl:apply-templates mode="RHseparationAnchorToDiv" select="exsl:node-set($temp4)"/>
    </xsl:variable>
    <xsl:variable name="temp6">
      <xsl:apply-templates mode="RHaddMissingRendKommentarInKommentar" select="exsl:node-set($temp5)"/>
    </xsl:variable>
    <xsl:variable name="temp7">
      <xsl:apply-templates mode="RHdeleteEmptyDiv" select="exsl:node-set($temp6)"/>
    </xsl:variable>
    <xsl:variable name="temp8">
      <xsl:apply-templates mode="RHrenameDivType" select="exsl:node-set($temp7)"/>
    </xsl:variable>
    <xsl:variable name="temp9">
      <xsl:apply-templates mode="RHaddNToPInDiv" select="exsl:node-set($temp8)"/>
    </xsl:variable>
    <xsl:variable name="temp10">
      <xsl:apply-templates mode="RHremoveN" select="exsl:node-set($temp9)"/>
    </xsl:variable>
    <xsl:apply-templates mode="RHaddMetadata" select="exsl:node-set($temp10)"/>
  </xsl:template>
